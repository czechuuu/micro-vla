...
\begin{itemize}
  \item We suspect that the default variance in initial block and nut locations, in the \textit{robosuite} environments is too low.
    We think that it contributes to the lack of diversity in the \textit{robomimic} datasets and lead to it being exploitable by simple heuristics that "hard-code" object positions - in essence, always performing the same trajectory.
  ...
  \item We observed that the transformer architecture can make use of prompt trajectories during training for sample efficient new task acquisition.
\end{itemize}
