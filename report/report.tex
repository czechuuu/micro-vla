...
A model trained on the block lifting task with a red cube exhibited equal zero-shot performance on cubes of different colors (see Figure~\ref{fig:multi_colour}).
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{Performance of the models trained with in-context prompts on the block lifting task.}
  \label{fig:icl_plots}
\end{figure}
...
This resulted in improved performance on the main lifting task, and a glimpse of generalization with nonzero, $\sim$ 10\%, stacking success rate (see Figure~\ref{fig:lift_and_stack}).
...
\begin{itemize}
  ...
  \item We believe that a decision transformer (at least in our implementation and at our scale) is incapable of truly in-context learning new tasks.
    However we are confident that training a new task requires orders of magnitude less demonstrations and this way enables in-context learning in a soft sense.
  \item TODO
\end{itemize}
